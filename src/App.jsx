...
function localBot(text, user, accounts) {
  ...
  return { action:"chat", reply:`Send me an Instagram link or @username to add it, or ask "is @username taken?"` };
}
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      if (s) { const p = JSON.parse(s); if (p?.username) { setUser(p); setMessages([{role:"assistant",text:`Welcome back, ${p.displayName}.`}]); } }
    } catch {}
  }, []);
  ...
  async function send() {
    ...
    if (action==="add" && username && canEdit(user.role)) {
      await fsSet("sources", username, {username, owner:user.displayName, role:user.role, addedAt:new Date().toISOString().slice(0,10), igLink:`https://www.instagram.com/${username}`, hot:"false"});
  ...
  const S = {
    page: { minHeight:"100vh", background:"#fafaf9", fontFamily:"'Geist','DM Sans',system-ui,sans-serif", color:"#18181b", display:"flex", flexDirection:"column" },
    ...
    btn: (active) => ({ background:active?"#18181b":"transparent", color:active?"#fff":"#71717a", border:"none", borderRadius:6, padding:"6px 12px", fontWeight:500, fontSize:13, cursor:"pointer", transition:"all .15s" }),
    input: { width:"100%", background:"#fff", border:"1px solid #e5e5e5", borderRadius:8, padding:"9px 12px", color:"#18181b", fontSize:13, outline:"none", fontFamily:"inherit", boxSizing:"border-box", transition:"border .15s" },
    tableHead: { display:"grid", padding:"0 16px", borderBottom:"1px solid #e5e5e5", background:"#fafaf9" },
    tableRow: { display:"grid", padding:"0 16px", borderBottom:"1px solid #f0f0f0", transition:"background .1s" },
    cell: { padding:"11px 8px", fontSize:13, display:"flex", alignItems:"center", overflow:"hidden" },
    tag: (c) => ({ background:c+"15", color:c, borderRadius:3, padding:"1px 6px", fontSize:11, fontWeight:600, whiteSpace:"nowrap" }),
  };

  // ── AUTH ──────────────────────────────────────────────────────
  if (!user) return (
    <div style={{ minHeight:"100vh", background:"#fafaf9", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", fontFamily:"'Geist','DM Sans',system-ui,sans-serif", padding:24 }}>
          ...
          <div>
            ...
            <div style={{ position:"relative" }}>
              <input type={showPass?"text":"password"} value={passInput} onChange={e=>{setPassInput(e.target.value);setAuthError("");}} onKeyDown={handleAuthKey}
                ...
                style={{...S.input, fontSize:14, paddingRight:40}} />
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.topbar}>
        <div style={{ display:"flex", alignItems:"center", gap:16 }}>
          ...
          <div style={{ display:"flex", gap:2 }}>
            {navItems.map(n=>(
              <button key={n.id} onClick={()=>setPage(n.id)} style={S.btn(page===n.id)}>{n.label}</button>
            ))}
          </div>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:6 }}>
            <div style={{ width:24, height:24, borderRadius:"50%", background:color, display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontWeight:700, fontSize:11 }}>{user.displayName[0].toUpperCase()}</div>
            <span style={{ fontSize:13, fontWeight:500, color:"#18181b" }}>{user.displayName}</span>
      ...
      {/* ── SOURCES — spreadsheet view ── */}
      {page==="sources" && (
        <div style={{ flex:1, display:"flex", flexDirection:"column" }}>
          {/* toolbar */}
          <div style={{ padding:"14px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12, borderBottom:"1px solid #e5e5e5", background:"#fff" }}>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <span style={{ fontSize:14, fontWeight:600, color:"#18181b" }}>{canSeeAll(user.role)?"All Sources":"My Sources"}</span>
              ...
              {sourceList.filter(a=>a.hot==="true").length > 0 && (
                <span style={{ fontSize:12, color:"#f97316", background:"#fff7ed", borderRadius:4, padding:"1px 7px", border:"1px solid #fed7aa" }}>🔥 {sourceList.filter(a=>a.hot==="true").length} hot</span>
              )}
            </div>
            <input
          ...
          {/* table header */}
          <div style={{ ...S.tableHead, gridTemplateColumns: canSeeAll(user.role) ? "2fr 1.2fr 1fr 1fr 80px" : "2fr 1fr 1fr 80px", background:"#fafaf9" }}>
            {["Account", ...(canSeeAll(user.role)?["Owner"]:[]), "Added", "Last visited", ""].map((h,i)=>(
              <div key={i} style={{ ...S.cell, fontSize:11, fontWeight:600, color:"#a1a1aa", textTransform:"uppercase", letterSpacing:.5, padding:"8px 8px" }}>{h}</div>
            ))}
          </div>
          ...
          <div style={{ flex:1, overflow:"auto" }}>
            {sourceList.length === 0 ? (
              ...
              return (
                <div key={a.id}
                  style={{ ...S.tableRow, gridTemplateColumns: canSeeAll(user.role) ? "2fr 1.2fr 1fr 1fr 80px" : "2fr 1fr 1fr 80px", background: idx%2===0?"#fff":"#fafaf9" }}
                ...
                >
                  ...
                  <div style={S.cell}>
                    <div style={{ display:"flex", alignItems:"center", gap:8, minWidth:0 }}>
                      ...
                      <a
                        ...
                        style={{ color:"#18181b", fontWeight:500, fontSize:13, textDecoration:"none", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", borderBottom:"1px solid #e5e5e5" }}
                        onMouseEnter={e=>e.target.style.borderBottomColor="#18181b"}
                        onMouseLeave={e=>e.target.style.borderBottomColor="#e5e5e5"}
                      >
                        instagram.com/{a.username}
                      </a>
                    </div>
                  </div>
                  {/* owner (admin only) */}
                  ...
                  {/* added */}
                  <div style={S.cell}>
                    <span style={{ fontSize:12, color:"#a1a1aa" }}>{a.addedAt||"—"}</span>
                  </div>
                  {/* last visited */}
                  <div style={S.cell}>
                    <span style={{ fontSize:12, color: lv?"#71717a":"#d4d4d4" }}>{lv ? timeAgo(lv) : "Never"}</span>
                  </div>
                  {/* actions */}
                  <div style={{...S.cell, gap:6, justifyContent:"flex-end"}}>
                    {canEdit(user.role) && (
                      <button onClick={()=>handleToggleHot(a.id, !isHot)} title={isHot?"Unmark hot":"Mark hot"} style={{ background:"none", border:"none", cursor:"pointer", fontSize:14, padding:"2px 4px", color:isHot?"#f97316":"#d4d4d4", transition:"color .15s" }}>🔥</button>
                    )}
                    {canRemoveAll(user.role) && (
                      <button onClick={()=>adminRemove(a.id)} title="Remove" style={{ background:"none", border:"none", cursor:"pointer", fontSize:13, padding:"2px 4px", color:"#d4d4d4", transition:"color .15s" }}
      ...
      {page==="browse" && canBrowse(user.role) && (
        <div style={{ flex:1, overflow:"auto", maxWidth:700, width:"100%", margin:"0 auto", padding:"24px 20px" }}>
          <div style={{ fontSize:14, fontWeight:600, color:"#18181b", marginBottom:4 }}>Browse Teammates</div>
          <div style={{ fontSize:13, color:"#a1a1aa", marginBottom:20 }}>Tap a name to see their sources</div>
          <div style={{ display:"flex", flexDirection:"column", gap:1, border:"1px solid #e5e5e5", borderRadius:10, overflow:"hidden", background:"#fff" }}>
            {users.filter(u=>u.role==="teammate").length === 0 ? (
              <div style={{ padding:"40px 20px", textAlign:"center", color:"#a1a1aa", fontSize:13 }}>No teammates signed up yet.</div>
              ...
              return (
                <div key={u.id} style={{ borderBottom: i<arr.length-1?"1px solid #f0f0f0":"none" }}>
                  <button onClick={()=>setBrowseExpanded(isOpen?null:u.id)} style={{ width:"100%", background:isOpen?"#fafaf9":"#fff", border:"none", cursor:"pointer", padding:"13px 16px", display:"flex", alignItems:"center", gap:12, textAlign:"left", transition:"background .15s" }}>
                    <div style={{ width:32, height:32, borderRadius:"50%", background:nc+"15", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700, fontSize:13, color:nc, flexShrink:0 }}>{u.displayName[0].toUpperCase()}</div>
                    <div style={{ flex:1 }}>
                      <div style={{ fontWeight:600, fontSize:13, color:"#18181b" }}>{u.displayName}</div>
                      <div style={{ fontSize:12, color:"#a1a1aa", marginTop:1 }}>
                        {uAccounts.length} source{uAccounts.length!==1?"s":""}{hotCount>0?` · ${hotCount} 🔥`:""}
                      </div>
                    </div>
                    <span style={{ color:"#d4d4d4", fontSize:12, transform:isOpen?"rotate(90deg)":"rotate(0deg)", transition:"transform .2s" }}>▶</span>
                  </button>
                  {isOpen && (
                    <div style={{ borderTop:"1px solid #f0f0f0", background:"#fafaf9" }}>
                      {uAccounts.length===0 ? (
                        <div style={{ padding:"16px", fontSize:13, color:"#a1a1aa" }}>No sources yet.</div>
                      ) : uAccounts.sort((a,b)=>(b.hot==="true"?1:0)-(a.hot==="true"?1:0)).map(a => {
                        const lv = a[`lastVisit_${u.username}`] || a.lastVisited;
                        return (
                          <div key={a.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 16px 10px 60px", borderBottom:"1px solid #f0f0f0" }}>
                            {a.hot==="true" && <span style={{ fontSize:12 }}>🔥</span>}
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", maxWidth:640, width:"100%", margin:"0 auto" }}>
          <div style={{ flex:1, overflow:"auto", padding:"20px 20px 8px" }}>
            {messages.map((m,i)=>(
              <div key={i} style={{ display:"flex", justifyContent:m.role==="user"?"flex-end":"flex-start", marginBottom:10, gap:8, alignItems:"flex-end" }}>
                {m.role==="assistant" && <div style={{ width:26, height:26, borderRadius:6, background:"#18181b", display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, color:"#fff", flexShrink:0, fontWeight:700 }}>CV</div>}
          ...
          <div style={{ display:"flex", gap:8, padding:"10px 20px 16px", borderTop:"1px solid #f0f0f0", background:"#fff" }}>
            <textarea ref={inputRef} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={handleChatKey}
              placeholder="Paste a link or @username…" rows={1}
              style={{ flex:1, background:"#fafaf9", border:"1px solid #e5e5e5", borderRadius:8, padding:"9px 12px", color:"#18181b", fontSize:13, resize:"none", fontFamily:"inherit", outline:"none", lineHeight:1.5 }} />
        ...
        <div style={{ flex:1, overflow:"auto", maxWidth:800, width:"100%", margin:"0 auto", padding:"24px 20px" }}>
          ...
          {adminTab==="accounts" && (
            <div style={{ border:"1px solid #e5e5e5", borderRadius:10, overflow:"hidden", background:"#fff" }}>
              <div style={{ display:"grid", gridTemplateColumns:"2fr 1.2fr 0.8fr 40px", borderBottom:"1px solid #e5e5e5", background:"#fafaf9" }}>
                {["Account","Owner","Added",""].map((h,i)=>(
              ...
              : accounts.sort((a,b)=>(b.addedAt||"").localeCompare(a.addedAt||"")).map((a,i,arr)=>(
                <div key={a.id} style={{ display:"grid", gridTemplateColumns:"2fr 1.2fr 0.8fr 40px", borderBottom:i<arr.length-1?"1px solid #f0f0f0":"none", background:i%2===0?"#fff":"#fafaf9" }}>
              ...
              : users.sort((a,b)=>{const o={owner:0,manager:1,linktree:2,teammate:3};return(o[a.role]||3)-(o[b.role]||3);}).map((u,i,arr)=>(
                <div key={u.id} style={{ display:"grid", gridTemplateColumns:"1.5fr 1fr 0.7fr 0.7fr", borderBottom:i<arr.length-1?"1px solid #f0f0f0":"none", background:i%2===0?"#fff":"#fafaf9" }}>
                  <div style={{ padding:"10px 12px", display:"flex", alignItems:"center", gap:8 }}>
                    ...
                    <span style={{ fontSize:13, fontWeight:500, color:"#18181b" }}>{u.displayName}</span>
                  </div>
      ...
      <style>{`@keyframes bounce{0%,100%{transform:translateY(0);opacity:.4}50%{transform:translateY(-4px);opacity:1}} a:hover{opacity:.85}`}</style>
    </div>
  );
}
